...
p <- plot_richness(CD_physeq, "treatment", measures=alpha_meas)
p1 <- p + geom_boxplot(data=p$data, aes(x=treatment, y=value, fill=treatment), outlier.colour = "black", alpha= 0.5, colour = "black")
plot1 = p1 + theme(text = element_text(size=12),axis.text.x = element_blank()) + labs(x = "")
plot1
ggsave(paste("figures/cd-alpha-div.png", sep = ""), plot1, height = 8.5, width = 11, units = "in")
...
for (metric in alpha_meas2) {
  ...
  ggsave(paste("figures/cd-time-series-", metric, "-alpha-div.png", sep = ""), time_series, height = 8.5, width = 11, units = "in")
}
...
ggsave(paste("figures/cd-bar-plot1.png", sep = ""), bar1, height = 8.5, width = 11, units = "in")
...
for (each_kind in uniques) {
  ...
  theme(panel.background = element_blank(),text = element_text(size=16))
  ggsave(paste("figures/cd-bar-", each_kind, ".png", sep = ""), bar2, height = 8.5, width = 11, units = "in")
}
...
plot2 = ord_1b + geom_point(size = 4) + theme_bw()
plot2
...
plot3 = ord_2b + geom_point(size = 4) + theme_bw()
plot3
